...
.mascot{width:100%;max-width:340px;padding:0 .5rem;margin-top:.15rem;}
.mascot svg{width:100%;height:auto;max-height:220px;overflow:visible;
  filter:drop-shadow(0 0 38px rgba(99,102,241,.2)) drop-shadow(0 0 65px rgba(236,72,153,.08));}
.title{text-align:center;padding:0 1rem;margin-top:-.2rem;}
.title h1{
  font-family:'JetBrains Mono',monospace;
  font-size:clamp(1.5rem,6vw,2.4rem);font-weight:800;
  color:#f1f5f9;letter-spacing:-.02em;line-height:1;margin-bottom:.15rem;
}
.o{color:transparent;-webkit-text-stroke:1.5px rgba(99,102,241,.75);}
.sub{font-size:.5rem;letter-spacing:.2em;color:rgba(148,163,184,.35);text-transform:uppercase;margin-bottom:.5rem;}
.pills{display:none;}
.divider{
  width:min(240px,80%);height:1px;margin:.1rem 0 .4rem;
...
.eyebrow{
  ...
  color:rgba(165,180,252,.32);text-align:center;margin-bottom:.2rem;
}
</style>
</head>
<body>
...
def auth_gate():
    ...
    # Shorter on mobile so the password field isn't buried below the fold —
    # the CSS inside _MASCOT_HTML shrinks the artwork on narrow viewports,
    # and this fixed height is sized for that compact mobile layout.
    components.html(_MASCOT_HTML, height=420, scrolling=False)
